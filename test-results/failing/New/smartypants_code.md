...
````````````html
<pre><p>&amp;</p></pre><code><p>--foo</p></code><kbd><p>---foo</p></kbd>&amp;lt;script&amp;gt;--foo&amp;lt;/script&amp;gt;<p>Ensure that text such as custom tags that happen to begin with the same letters as the above tags don&#39;t match and thus benefit from Smartypants-ing.</p><script-custom><p>--foo</p></script-custom><p><code>--foo</code>&lt;codebar --foo codebar&gt;</p>
````````````
